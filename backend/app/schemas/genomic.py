...
from pydantic import BaseModel, Field, UUID4, field_validator, model_validator, ConfigDict
...
class GenomicProfileIngest(BaseModel):
    ...
    latitude: Optional[float] = Field(default=None, description="Decimal latitude for geospatial analysis")
    longitude: Optional[float] = Field(default=None, description="Decimal longitude for geospatial analysis")
...
# ═══════════════════════════════════════════════════════════════════════════════
# GEO-FORENSIC ANCESTRY SCHEMAS
# ═══════════════════════════════════════════════════════════════════════════════

class GeoProbabilityModel(BaseModel):
    """Geographic ancestry probability for a single reference population."""
    region: str = Field(..., description="Reference population name")
    lat: float = Field(..., description="Latitude of region centroid")
    lng: float = Field(..., description="Longitude of region centroid")
    probability: float = Field(..., ge=0.0, le=1.0, description="Normalized probability score")
    color: str = Field(default="#22C55E", description="Display color for this region")
